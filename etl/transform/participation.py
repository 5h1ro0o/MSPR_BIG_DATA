...
def transform_participation(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    commune_tour = (
        pres.groupby(
            ["code_commune", "code_departement", "libelle_departement",
             "libelle_commune", "id_election"],
            as_index=False,
        ).agg(AGG_COLS)
    )

    commune_tour["taux_abstention"] = pct(commune_tour["abstentions"], commune_tour["inscrits"])
    commune_tour["taux_participation"] = pct(commune_tour["votants"], commune_tour["inscrits"])
    commune_tour["taux_blancs_votants"] = pct(commune_tour["blancs"], commune_tour["votants"])
    commune_tour["taux_nuls_votants"] = pct(commune_tour["nuls"], commune_tour["votants"])
    commune_tour["taux_exprimes_votants"] = pct(commune_tour["exprimes"], commune_tour["votants"])

    def suffix_cols(df_tour: pd.DataFrame, suffix: str) -> pd.DataFrame:
        numeric = [
            "inscrits", "abstentions", "votants", "blancs", "nuls", "exprimes",
            "taux_abstention", "taux_participation",
            "taux_blancs_votants", "taux_nuls_votants", "taux_exprimes_votants",
        ]
    ...
    t1 = suffix_cols(commune_tour[commune_tour["id_election"] == "2022_pres_t1"].copy(), "t1")
    t2 = suffix_cols(commune_tour[commune_tour["id_election"] == "2022_pres_t2"].copy(), "t2")
