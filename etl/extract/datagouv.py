...
def _fetch_elections(data_root: Path) -> bool:
    ...
    if all_candidats:
        ...
        log.info(f"  Candidats : {len(candidats_full):,} lignes → {candidats_file.name}")
...
_COL_ALIASES = {
    "code_departement":   ["code du département", "code département", "code departement",
                           "num_dpt", "coddpt", "dept"],
    "libelle_departement":["libellé du département", "libelle département", "libelle departement",
                           "lib_dpt"],
    "code_commune":       ["code de la commune", "code commune", "codecomm", "com",
                           "codgeo", "code_commune"],
    "libelle_commune":    ["libellé de la commune", "libelle commune", "lib_commune",
...
def _parse_election_csv(
    ...
    df["code_departement"] = (
        df["code_departement"].astype(str).str.strip()
    ...
    part_cols = ["code_departement", "libelle_departement", "code_commune",
                 "libelle_commune", "inscrits", "abstentions", "votants",
    ...
    if "abstentions" not in participation.columns:
        participation["abstentions"] = participation["inscrits"] - participation["votants"]
    ...
    part = participation
    part["ratio_abstentions_inscrits"] = safe_pct(part.get("abstentions", 0), part["inscrits"])
    part["ratio_votants_inscrits"]     = safe_pct(part.get("votants", 0),     part["inscrits"])
    part["ratio_exprimes_inscrits"]    = safe_pct(part.get("exprimes", 0),    part["inscrits"])
    part["ratio_exprimes_votants"]     = safe_pct(part.get("exprimes", 0),    part.get("votants", part["inscrits"]))

    candidats = None
    if "nom" in df_idf.columns and "voix" in df_idf.columns:
        cand_cols = ["code_departement", "code_commune", "nom",
                     "prenom" if "prenom" in df_idf.columns else "nom", "voix"]
        cand = df_idf[[c for c in cand_cols if c in df_idf.columns]].copy()
        cand["id_election"] = id_election
        cand = cand.rename(columns={"code_departement": "code_departement",
                                    "code_commune":     "code_commune"})
        if "prenom" not in cand.columns:
            cand["prenom"] = ""
        candidats = cand[["id_election", "code_departement", "code_commune",
                           "voix", "nom", "prenom"]]

    log.info(f"    {id_election} : {len(participation):,} communes IDF"
...
def _get_with_retry(url: str, max_retries: int = 3, timeout: int = 120) -> requests.Response:
