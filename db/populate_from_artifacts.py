...
PRED_FILES = [
    ("gradient_boosting", "post_t1",  "regression_macron", "gb_predictions_post_t1_regression_macron.csv"),
    ("gradient_boosting", "pre_vote", "regression_macron", "gb_predictions_pre_vote_regression_macron.csv"),
    ("random_forest",     "post_t1",  "regression_macron", "rf_predictions_post_t1_regression_macron.csv"),
    ("random_forest",     "pre_vote", "regression_macron", "rf_predictions_pre_vote_regression_macron.csv"),
    ("decision_tree",     "pre_vote", "regression_macron", "dt_predictions_pre_vote_regression_macron.csv"),
    ("mlp",               "pre_vote", "regression_macron", "mlp_predictions_pre_vote_regression_macron.csv"),
    ("lstm",              "lstm",     "regression_macron", "lstm_predictions_regression_macron.csv"),
]

METRICS_FILES = [
    ("gradient_boosting", "post_t1",  "regression_macron", "gradient_boosting_metrics_post_t1_regression_macron.json"),
    ("gradient_boosting", "pre_vote", "regression_macron", "gradient_boosting_metrics_pre_vote_regression_macron.json"),
    ("random_forest",     "post_t1",  "regression_macron", "random_forest_metrics_post_t1_regression_macron.json"),
    ("random_forest",     "pre_vote", "regression_macron", "random_forest_metrics_pre_vote_regression_macron.json"),
    ("decision_tree",     "pre_vote", "regression_macron", "decision_tree_metrics_pre_vote_regression_macron.json"),
    ("mlp",               "pre_vote", "regression_macron", "mlp_metrics_pre_vote_regression_macron.json"),
...
def populate_model_predictions(engine, run_id: str) -> None:
    ...
    for model_name, feature_set, target, fname in PRED_FILES:
        ...
        with engine.begin() as conn:
            ...
            conn.execute(
                text("DELETE FROM gold.model_predictions WHERE model_name=:m AND feature_set=:f AND target=:t"),
                {"m": model_name, "f": feature_set, "t": target},
            )
        df_out.to_sql("model_predictions", schema="gold", con=engine,
                      if_exists="append", index=False, method="multi", chunksize=500)
        ok.append(f"{model_name}/{feature_set}")
    log.info(f"  model_predictions : {len(ok)} modèles insérés" + (f" · {len(skipped)} SKIP" if skipped else ""))
    if skipped:
    ...
    for model_name, feature_set, target, fname in METRICS_FILES:
        ...
        for k, v in m.items():
            if isinstance(v, (int, float)):
                rows.append({"run_id": run_id, "model_name": model_name,
                              "feature_set": feature_set, "target": target,
                              "metric_name": k, "metric_value": float(v)})
        if not rows:
            continue
        ...
        with engine.begin() as conn:
            conn.execute(text(_CREATE_GOLD_SCHEMA))
            conn.execute(
                text("DELETE FROM gold.model_metrics WHERE model_name=:m AND feature_set=:f AND target=:t"),
                {"m": model_name, "f": feature_set, "t": target},
            )
        df.to_sql("model_metrics", schema="gold", con=engine,
                  if_exists="append", index=False, method="multi", chunksize=200)
        ok.append(f"{model_name}/{feature_set}")
    log.info(f"  model_metrics : {len(ok)} modèles insérés" + (f" · {len(skipped)} SKIP" if skipped else ""))
    if skipped:
        log.warning(f"  Fichiers introuvables : {skipped}")
...
def populate_ml_features(engine, csv_path: Path) -> None:
    ...
    with engine.begin() as conn:
        conn.execute(text(_CREATE_GOLD_SCHEMA))
    df.to_sql("ml_features", schema="gold", con=engine,
...
def main():
    ...
    run_id = datetime.datetime.now(datetime.timezone.utc).strftime("%Y%m%dT%H%M%S") + "_artifacts"

    gold_csv = Path(os.environ.get("DATA_PATH", "/data/dataset_elections_2022_idf.csv"))
    if not gold_csv.exists():
        gold_csv = Path(os.environ.get("DATA_ROOT", "/data")) / "dataset_elections_2022_idf.csv"
