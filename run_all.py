...
def step_fetch(data_root: Path) -> bool:
    ...
    try:
        ...
        log.info(f"Fetch terminé en {time.time()-t0:.1f}s — {ok_count}/{len(results)} datasets OK")
        return True
...
def step_train() -> bool:
    ...
    try:
        ...
        for fset in ("pre_vote", "post_t1"):
            ...
            try:
                m = train_random_forest(feature_set=fset, target="classification_t2",
                                        fast_search=True, n_iter=20)
            ...
            try:
                m = train_gradient_boosting(feature_set=fset, target="classification_t2",
                                            use_search=False)
        ...
        log.info(f"Entraînement terminé en {time.time()-t0:.1f}s — {len(results)} modèles")
        return True
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--skip-fetch", action="store_true", help="Saute le fetch data.gouv.fr")
    parser.add_argument("--skip-etl",   action="store_true", help="Saute le pipeline ETL")
    parser.add_argument("--skip-train", action="store_true", help="Saute l'entraînement ML")
