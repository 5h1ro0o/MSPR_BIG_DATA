...
def plotly_radar_comparison(
    ...
    present = [m for m in metrics if any(m in v for v in results.values())]
    if not present:
        fig = go.Figure()
...
def plotly_predictions_map(
    ...
    summary = pred_df.groupby(["dept", "correct"]).size().reset_index(name="count")
