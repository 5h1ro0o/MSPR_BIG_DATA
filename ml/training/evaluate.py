...
def bootstrap_ci(
    ...
    return round(float(np.percentile(scores, alpha * 100)), 4), round(
        float(np.percentile(scores, (1 - alpha) * 100)), 4
    )
...
def baseline_metrics(y_true: np.ndarray) -> dict:
    ...
    return {
        ...
        "baseline_balanced_accuracy": round(
            balanced_accuracy_score(y_true, y_pred_baseline), 4
        ),
        "baseline_f1_weighted": round(
            f1_score(y_true, y_pred_baseline, average="weighted", zero_division=0), 4
        ),
        "class_distribution": {
            int(c): round(int(cnt) / n, 4) for c, cnt in zip(classes, counts)
        },
    }
...
def plot_roc_curve(y_true, y_proba, model_name, output_dir) -> Path:
    ...
    ax.set(
        xlabel="Faux Positifs (1 − Spécificité)",
        ylabel="Vrais Positifs (Sensibilité)",
...
def plot_calibration(y_true, y_proba, model_name: str, output_dir: Path) -> Path:
    ...
    ax.set(
        xlabel="Probabilité prédite (Le Pen)",
...
def plot_predictions_vs_actual(
    ...
    lim = [
        min(float(min(y_true)), float(min(y_pred))) - 1,
