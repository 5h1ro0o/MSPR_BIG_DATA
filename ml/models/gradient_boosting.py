...
from sklearn.model_selection import (
    RandomizedSearchCV, cross_val_score, StratifiedKFold, KFold,
)
...
from ml.config import (
    ...
    RANDOM_STATE, CV_FOLDS, ARTIFACTS, LEAKAGE_KEYWORDS,
)
...
class GradientBoostingModel(BaseModel):
    ...
    def filter_leakage(feature_names: list[str]) -> list[str]:
        ...
        return [
            ...
            if not any(kw in f for kw in LEAKAGE_KEYWORDS)
        ]
    ...
    def train(
        ...
        self.cv_scores_ = cross_val_score(self.pipeline, X_tr, y_train, cv=cv, scoring=scoring)
        print(f"  CV {scoring} : {self.cv_scores_.mean():.4f} ± {self.cv_scores_.std():.4f}")
    ...
    def _run_search(self, X_tr, y_train, n_iter: int) -> Pipeline:
        ...
        search = RandomizedSearchCV(
            base, grid, n_iter=n_iter, scoring=scoring,
            cv=cv, n_jobs=-1, random_state=RANDOM_STATE, refit=True, verbose=0,
        )
        search.fit(X_tr, y_train)
        self.best_params_ = {k.replace("gb__", ""): v for k, v in search.best_params_.items()}
    ...
    def _compute_metrics(self, y_true, y_pred, split: str) -> dict:
        ...
        if self.task == "classification":
            return {
                ...
                f"{p}f1":        round(f1_score(y_true, y_pred, average="weighted", zero_division=0), 4),
                f"{p}precision": round(precision_score(y_true, y_pred, average="weighted", zero_division=0), 4),
                f"{p}recall":    round(recall_score(y_true, y_pred, average="weighted", zero_division=0), 4),
    ...
    def plot_results(
        ...
        fig.suptitle("Gradient Boosting — Élections Présidentielles 2022 IDF",
        ...
        ax = axes[1, 1]
        ax.hist(y_proba_all[y_all == 0], bins=30, alpha=0.6,
                color="steelblue", label="Macron (réel)", density=True)
        ax.hist(y_proba_all[y_all == 1], bins=30, alpha=0.6,
                color="crimson", label="Le Pen (réel)", density=True)
